...
A production-ready, read-only AWS cost optimization scanner that analyzes **36 AWS service categories** across **all AWS regions** with **260+ automated checks**. Outputs JSON scan results and an audit-grade HTML report (Newsreader + IBM Plex type system, sticky savings-sorted jump-nav rail, source-confidence taxonomy, structured executive summary, and dark mode).
...
- **Per-type commitment purchase advisor** — the Commitment Analysis tab covers every CE-supported Reserved Instance / Savings Plan type (EC2, RDS, ElastiCache, Redshift, OpenSearch, DynamoDB RIs; Compute, EC2-Instance, and SageMaker Savings Plans) with a full 1yr/3yr x No/Partial/All Upfront scenario matrix per instance type, break-even and risk-of-loss math, existing-coverage context, an EC2 Savings-Plan-vs-Reserved-Instance comparison, and Cost Optimization Hub concurrence. A separate "Projected commitment savings" executive-summary fact reports the best non-overlapping purchase path — always advisory, never folded into the counted headline.
...
| Commitment Analysis | `commitment_analysis`, `commitments`, `savings_plans`, `ri` | `commitment_analysis.py` | Savings Plans utilization/coverage, RI utilization/coverage, per-instance-type purchase advisor across all 6 CE-supported RI services (EC2, RDS, ElastiCache, Redshift, OpenSearch, DynamoDB) and all 3 SP types (Compute, EC2-Instance, SageMaker): full (term × payment) scenario matrix per type, break-even/risk math, coverage context, EC2 SP-vs-RI comparison, projected best-path savings fact; absorbs Cost Optimization Hub RI / SP findings via `ctx.cost_hub_splits` |
