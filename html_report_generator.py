...
<body>
    ...
    def _get_tabs(self) -> str:
        ...
        tab_buttons += '<button class="tab-button active" onclick="showTab(\'executive-summary\')">📊 Executive Summary</button>'
        ...
        if snapshots_data['count'] > 0 and not any(s.get('total_recommendations', 0) > 0 for k, s in services.items() if k == 'ebs'):
            tab_buttons += f'<button class="tab-button" onclick="showTab(\'snapshots\')">Snapshots</button>'
        
        # Add standalone AMIs tab if no AMI service but AMIs exist
        if amis_data['count'] > 0 and not any(s.get('total_recommendations', 0) > 0 for k, s in services.items() if k == 'ami'):
    ...
    def _extract_snapshots_data(self, services: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_savings': sum(
                ...
                if 'EstimatedSavings' in rec and rec.get('EstimatedSavings', '0') != '0' 
                and rec.get('EstimatedSavings', '').replace('$', '').replace('/month', '').split('(')[0].strip().replace('.', '').isdigit()
            )
        }
    ...
    def _get_snapshots_content(self, snapshots_data: Dict[str, Any]) -> str:
        ...
        content += f'<div class="stat-card"><h4>Old Snapshots</h4><div class="value">{snapshots_data["count"]}</div></div>'
        ...
        content += f'<strong>Estimated Monthly Savings:</strong> <span class="savings">${total_savings:.2f}</span></div>'
        ...
        for age_range, snapshots in age_groups.items():
            ...
            content += f'<p><strong>Recommendation:</strong> Review and delete old snapshots that are no longer needed</p>'
        ...
        <div class="service-header">
            ...
            <div class="service-stats">
                <div class="stat-card">
                    ...
                    <div class="value savings">$''' + f'{total_savings:.2f}' + '''</div>
                </div>
                <div class="stat-card">
                    <h4>Total Recommendations</h4>
                    <div class="value">''' + str(total_recommendations) + '''</div>
                </div>
                <div class="stat-card">
                    <h4>Services Scanned</h4>
                    <div class="value">''' + str(services_scanned) + '''</div>
                </div>
            </div>
        </div>
            ...
            for source_name, source_data in filtered_data['sources'].items():
                if 'recommendations' in source_data:
                    ...
                    if source_name == 'cost_optimization_hub':
                        ...
                        filtered_data['total_monthly_savings'] = max(0, 
                            filtered_data.get('total_monthly_savings', 0) - graviton_savings)
            ...
            for rec in recommendations:
                if service_key == 'ec2':
                    ...
                    if 'actionType' in rec:
                        ...
                        if ('ecs-cluster' in all_text or 
                            '/cronjob' in all_text or
                            'forwarder' in all_text or
                            'lambda' in all_text or
                            'ecs' in all_text):
                            continue
                        ...
                        instance_type = rec.get('currentResourceDetails', {}).get('ec2Instance', {}).get('configuration', {}).get('instance', {}).get('type', 'N/A')
                    ...
                    if 'actionType' in rec and 'ebsVolume' in rec.get('currentResourceDetails', {}):
                        ...
                        resource_groups[action_type].append({
                            'id': resource_id,
                            'type': f"{volume_type} ({volume_size} GB)",
                            'savings': savings
                        })
                    ...
                    finding = (rec.get('instanceFinding') or 
                              rec.get('InstanceFinding') or 
                              rec.get('finding') or
                              rec.get('CheckCategory') or 
    ...
    def _get_service_content(self, service_key: str, service_data: Dict[str, Any]) -> str:
        ...
        if service_key not in ['ebs', 'ec2', 'rds', 's3', 'dynamodb', 'containers', 'elasticache', 'opensearch', 'file_systems', 'network', 'monitoring', 'additional_services', 'lambda', 'cloudfront', 'api_gateway', 'step_functions', 'auto_scaling', 'backup', 'route53', 'ami', 'lightsail', 'dms', 'glue', 'redshift']:
    ...
    def _get_service_stats(self, service_key: str, service_data: Dict[str, Any]) -> str:
        ...
        if service_key == 'ec2':
            ...
            stats_html += f'<div class="stat-card"><h4>Total Volumes</h4><div class="value">{counts.get("total", 0)}</div></div>'
            stats_html += f'<div class="stat-card"><h4>Unattached</h4><div class="value">{counts.get("unattached", 0)}</div></div>'
            stats_html += f'<div class="stat-card"><h4>gp2 Volumes</h4><div class="value">{counts.get("gp2", 0)}</div></div>'
            has_stats = True
            ...
            stats_html += f'<div class="stat-card"><h4>Running</h4><div class="value">{counts.get("running", 0)}</div></div>'
            stats_html += f'<div class="stat-card"><h4>MySQL</h4><div class="value">{counts.get("mysql", 0)}</div></div>'
            has_stats = True
            ...
            stats_html += f'<div class="stat-card"><h4>Total Buckets</h4><div class="value">{counts.get("total", 0)}</div></div>'
            ...
            stats_html += f'<div class="stat-card"><h4>Total Tables</h4><div class="value">{counts.get("total", 0)}</div></div>'
            stats_html += f'<div class="stat-card"><h4>Provisioned</h4><div class="value">{counts.get("provisioned", 0)}</div></div>'
            stats_html += f'<div class="stat-card"><h4>On-Demand</h4><div class="value">{counts.get("on_demand", 0)}</div></div>'
            has_stats = True
    ...
    def _get_detailed_recommendations(self, service_key: str, service_data: Dict[str, Any]) -> str:
        ...
        if service_key == 'file_systems':
            ...
            for category, functions in grouped_lambda.items():
                ...
                for func in functions:
                    ...
                    if 'currentResourceDetails' in func:
                        lambda_config = func.get('currentResourceDetails', {}).get('lambdaFunction', {}).get('configuration', {})
            ...
            for category, resources in grouped_rds.items():
                ...
                for res in resources:
                    ...
                    resource_id = (res.get('DBInstanceIdentifier') or 
                                 res.get('DBClusterIdentifier') or 
                                 res.get('dbClusterIdentifier') or  # Compute Optimizer format
                                 res.get('dbInstanceIdentifier') or  # Compute Optimizer format
                                 res.get('SnapshotId') or 
                                 res.get('ResourceId') or
                                 res.get('resourceArn', '').split(':')[-1] if res.get('resourceArn') else 'Unknown')
            ...
            for category, resources in grouped_resources.items():
                ...
                for res in resources:
                    ...
                    if service_key == 'api_gateway':
                        ...
                        resource_id = res.get('StateMachineArn', 'Unknown').split(':')[-1] if res.get('StateMachineArn') else res.get('StateMachineName', 'Unknown')
                    elif service_key == 'auto_scaling':
                        resource_id = res.get('AutoScalingGroupName', res.get('GroupName', 'Unknown'))
                    elif service_key == 'backup':
                        resource_id = res.get('BackupPlanName', res.get('BackupVaultName', res.get('PlanName', 'Unknown')))
                        ...
                        resource_id = res.get('AlarmName', res.get('LogGroupName', res.get('TrailName', res.get('Namespace', res.get('HostedZoneId', res.get('HealthCheckId', 'Unknown'))))))
        ...
        for source_name, source_data in sources.items():
            ...
            if count > 0:
                ...
                if service_key not in ['ebs', 'ec2', 's3', 'dynamodb', 'containers', 'elasticache', 'opensearch', 'file_systems', 'network', 'monitoring', 'additional_services', 'rds', 'lightsail', 'dms', 'glue', 'redshift']:
                ...
                if service_key == 'ec2' and source_name == 'enhanced_checks':
                    ...
                    for rec in recommendations:
                        ...
                        resource_id = rec.get("InstanceId", rec.get("ImageId", rec.get("AllocationId", rec.get("ResourceId", rec.get("resourceId", "Resource")))))
                        resource_name = rec.get("Name", rec.get("ResourceName", ""))
                ...
                if service_key == 'ec2' and source_name == 'cost_optimization_hub':
                    ...
                    for action, recs in grouped_actions.items():
                        ...
                        for rec in recs:
                            resource_id = rec.get('resourceId', 'N/A')
                            current_type = rec.get('currentResourceDetails', {}).get('ec2Instance', {}).get('configuration', {}).get('instance', {}).get('type', 'N/A')
                            rec_type = rec.get('recommendedResourceDetails', {}).get('ec2Instance', {}).get('configuration', {}).get('instance', {}).get('type', 'N/A')
                            savings = rec.get('estimatedMonthlySavings', 0)
                    ...
                    for category, resources in grouped_network.items():
                        ...
                        for res in resources:
                            ...
                            if not resource_name:
                                resource_id = (res.get('AllocationId') or res.get('NatGatewayId') or 
                                             res.get('LoadBalancerName') or res.get('VpcEndpointId') or 
                                             res.get('VpcId') or res.get('AutoScalingGroupName') or
                                             res.get('InstanceId') or  # Add EC2 instance support
                                             (f"{res['ALBCount']} ALBs" if res.get('ALBCount') else None) or
                                             (f"{res['BackupPlanCount']} backup plans" if res.get('BackupPlanCount') else None) or
                                             'Unknown')
                                ...
                                if resource_id.startswith('eipalloc-'):
                                    ...
                                    if res.get('ServiceName'):
                                        service_name = res.get('ServiceName', '').split('.')[-1]  # Get service name (e.g., 's3', 'ec2')
                    ...
                    for category, resources in grouped_monitoring.items():
                        ...
                        for res in resources:
                            ...
                            if 'TrailNames' in res and isinstance(res['TrailNames'], list):
                                ...
                                resource_id = (res.get('LogGroupName') or res.get('TrailName') or 
                                             res.get('AlarmName') or res.get('Namespace') or
                                             res.get('BackupPlanName') or res.get('HostedZoneId') or
                                             res.get('HealthCheckId') or
                                             (f"{res['BackupPlanCount']} backup plans" if res.get('BackupPlanCount') else None) or
                                             'Unknown')
                            content += f'<li>{resource_id}</li>'
                        content += '</ul></div>'
                ...
                if service_key == 'additional_services':
                    ...
                    for category, resources in grouped_additional.items():
                        ...
                        content += '<p><strong>Resources:</strong></p><ul>'
                        for res in resources:
                            resource_id = res.get('DistributionId', res.get('ApiId', res.get('StateMachineArn', res.get('FunctionName', 'Unknown'))))
                ...
                if service_key in ['ebs', 'ec2', 's3', 'dynamodb', 'containers', 'file_systems', 'network', 'monitoring', 'additional_services']:
                    continue
                ...
                for rec in recommendations:
                    ...
                    if service_key == 'ec2':
                        ...
                        resource_id = (rec.get('LoadBalancerName') or rec.get('AutoScalingGroupName') or 
                                     rec.get('VpcEndpointId') or rec.get('NatGatewayId') or 
                                     rec.get('AllocationId') or rec.get('LogGroupName') or
                                     rec.get('TrailName') or rec.get('FunctionName') or
                                     rec.get('DistributionId') or rec.get('ApiId') or
                                     rec.get('VpcId') or rec.get('StateMachineArn', '').split(':')[-1] or
                                     rec.get('BackupPlanName') or rec.get('BackupVaultName') or
                                     rec.get('HostedZoneId') or rec.get('HealthCheckId') or
                                     rec.get('GroupName') or rec.get('PlanName') or
                                     rec.get('ResourceId') or rec.get('SnapshotId') or
                                     rec.get('dbClusterIdentifier') or rec.get('dbInstanceIdentifier') or
                                     (f"{rec['BackupPlanCount']} backup plans" if rec.get('BackupPlanCount') else None) or
                                     (f"{rec['ALBCount']} ALBs" if rec.get('ALBCount') else None) or
                                     rec.get('resourceArn', '').split(':')[-1] if rec.get('resourceArn') else 'Resource')
    ...
    def _get_savings_plans_content(self, sp_data: Dict[str, Any]) -> str:
        ...
        if utilization:
            ...
            status_class = 'success' if util_status == 'Good' else 'warning' if util_status == 'Needs Attention' else 'danger'
        ...
        if coverage:
            ...
            content += f'<div class="stat-card"><h4>Coverage</h4><div class="value {status_class}">{cov_pct:.1f}%</div></div>'
        ...
        if recommendations:
            ...
            for rec in recommendations:
                ...
                if 'potential_monthly_savings' in rec:
                    content += f'<p class="savings"><strong>Potential Savings:</strong> {rec["potential_monthly_savings"]}</p>'
...
if __name__ == "__main__":
    ...
    html_file = generate_html_report_from_json(json_file, output_file)
