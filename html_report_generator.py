...
# Sprite reference for the Executive Summary heading, hoisted so the two
# empty-state branches (LS-6) share one definition.
_EXEC_SUMMARY_ICON = '<svg class="icon icon-sm"><use href="#icon-chart"/></svg>'
...
<symbol id="icon-download" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></symbol>
    ...
    def _get_executive_summary_content(self) -> str:
        ...
        advisory = int(summary.get("total_advisory_recommendations", 0) or 0)
        if total_recommendations == 0:
            # LS-6 — "well-optimized" is an affirmative claim the scan did not
            # make, and rendering it directly above N advisory cards reads as a
            # contradiction. With advisories present, say what they are instead:
            # findings that render but carry no defensible account-specific
            # dollar, so they are never summed into the headline.
            if advisory > 0:
                plural = "s" if advisory != 1 else ""
                empty_state = (
                    "<h3>No Quantified Savings Identified</h3>"
                    "<p>No recommendation on this account resolved to a concrete, "
                    "account-specific dollar saving, so the counted total is $0.00. "
                    f"The {advisory} advisory finding{plural} below did render: each names "
                    "a real cost lever whose saving this scan could not size from the "
                    "available data, so none is counted toward the total.</p>"
                )
            else:
                empty_state = (
                    "<h3>No Cost Optimization Recommendations Found</h3>"
                    "<p>Your AWS resources appear to be well-optimized. No immediate "
                    "cost savings opportunities were identified.</p>"
                )
            return f"""
            <div class="service-header">
                <h2 class="service-title">{_EXEC_SUMMARY_ICON} Executive Summary</h2>
            </div>
    ...
    def _get_footer(self) -> str:
        ...
        # LS-6 — the counted total alone read as "nothing was found" beside a
        # page of advisory cards. Name them here too, from the same source of
        # truth the executive summary uses.
        advisory = int(summary.get("total_advisory_recommendations", 0) or 0)
        advisory_note = f" and {advisory} advisory finding{'s' if advisory != 1 else ''}" if advisory else ""
        return f"""<div class="footer">
            <p>Generated by AWS Cost Optimization Scanner on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
            <p>Report covers {summary["total_services_scanned"]} AWS services with {summary["total_recommendations"]} optimization recommendations{advisory_note}</p>
            {download_link}
        </div>"""
