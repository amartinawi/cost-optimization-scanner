...
| `rds.py` | `get_rds_instance_monthly_price()` (engine + Multi-AZ price delta), `get_rds_backup_storage_price_per_gb()`; Compute Optimizer via `compute_optimizer_savings()`; **consumes Cost Hub** via `ctx.cost_hub_splits["rds"]`. Cross-source dedup + RI demotion live in `services/rds_logic.py`; opt-in placeholder→warning mirrors EC2. (No gp2→gp3 check — RDS gp2 and gp3 base storage are the same price.) | AWS Pricing API + Cost Optimization Hub |
